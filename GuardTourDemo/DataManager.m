...
NSString * const kDataManagerModelName = @"GuardPatrol";
NSString * const kDataManagerSQLiteName = @"GuardPatrol.sqlite";
...
{   
    ...
    fetchResults = [self.mainObjectContext executeFetchRequest:fetchRequest error: &error];
    if (error != nil) 
    {
		NSLog(@"Error while executing fetch request: %@\n%@", [error localizedDescription], [error userInfo]);
...
{   
    ...
    fetchCount = [self.mainObjectContext countForFetchRequest:fetchRequest error: &error];
    if (error != nil) 
    {
		NSLog(@"Error while executing fetch request: %@\n%@", [error localizedDescription], [error userInfo]);
        //		[[NSNotificationCenter defaultCenter] postNotificationName:DataManagerDidSaveFailedNotification object:error];
...
- (NSString*)sharedDocumentsPath {
	...
	SharedDocumentsPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
